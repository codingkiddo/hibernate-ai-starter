...
public class VectorDao {
    ...
    public VectorDao(JdbcTemplate jdbc) { this.jdbc = jdbc; }
    ...
    public List<Product> semanticSearch(float[] queryEmbedding, int k) {
        String sql = "SELECT id, name, description, metadata FROM product ORDER BY embedding <=> ? ASC LIMIT ?";
    ...
    public List<Product> hybridSearch(float[] queryEmbedding, String query, int k, double wVec, double wFts) {
        String sql = "SELECT id, name, description, metadata FROM product ORDER BY ( " + 
              " ? * ts_rank_cd( " + 
